...
# This class represents a Chat message in RethinkDB.
class KOChat(Model):
    # Convenience method
    @classmethod
    def make(cls, source, name, message):
        """
        Create a new KOChat message entry.
        Remodel itself has a create() call, we need to provide data for it to store.
        @return: A KOChat object instantiated from the supplied data or None.
        """
        meta = {'source': source, 'name': name, 'updated_at': r.now()}
        print('KOCHATMODEL: Make: metadata: {}, message:\n{} '.format(meta, message))
        try:  # To make the database entry with the meta data.
            the_new_message = KOChat.create(meta=meta, message=message) 
        except KeyError:
            return None
        print('KOCHATMODEL: Make SUCCESS: metadata: {}, message:\n{} '.format(meta, message))
        return the_new_message

    def __repr__(self):
        return '<KOChat message {} Source: {} Name: {}>'.format(
