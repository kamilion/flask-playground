...
class Chat(Model):
    ...
    def make(cls, source, name, message):
        ...
        @return: A Chat object instantiated from the supplied data or None.
        """
        meta = {'source': source, 'name': name, 'updated_at': r.now()}
        print('CHATMODEL: Make: metadata: {}, message:\n{} '.format(meta, message))
        try:  # To make the database entry with the meta data.
            the_new_message = Chat.create(meta=meta, message=message) 
        except KeyError:
            return None
        print('CHATMODEL: Make SUCCESS: metadata: {}, message:\n{} '.format(meta, message))
        return the_new_message

    def __repr__(self):
        return '<Chat message {} Source: {} Name: {}>'.format(
